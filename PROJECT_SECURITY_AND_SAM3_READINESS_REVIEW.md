...
> 2026-06-21 修复状态：**已完成核心整改**。Redis 宿主机端口固定绑定 `127.0.0.1`，容器网络内使用
> 强制密码；密码通过独立的 `REDIS_PASSWORD` 注入，不再写入 URL。API Queue 与 Worker 均固定使用
> `JSONSerializer`，并增加部署配置和序列化器回归测试。升级前需先清空旧的 Pickle 格式待执行队列。
...
> 2026-06-21 修复状态：**认证与网络发布边界已完成整改**。默认发布地址改为 `127.0.0.1`；非回环地址或
> 生产环境会强制要求 API Key 模式。API Key 只用于换取短期 HttpOnly 会话，写请求执行 CSRF 校验，
> `/api/*` 与 `/media/*` 均受保护；仅 `/healthz` 和认证引导接口公开。已删除按客户端回环 IP 绕过
> 全部认证的逻辑，并增加密钥强度、CORS、Host、HTTPS Cookie 和危险配置启动校验。
>
> 本项不包含通用业务限流与磁盘配额；它们仍属于后续独立容量治理任务。
...
1. ~~关闭 Redis 对外暴露，启用密码，并把 RQ 改为 JSON serializer。~~ 已完成。
2. API 鉴权、CORS/Host 白名单与默认回环发布已完成；通用限流和磁盘配额待后续容量治理。
...
- [x] Redis 不对外暴露且启用密码，RQ 不再使用 Pickle。
- [x] API 已鉴权且默认不监听非回环地址。
- [ ] API 已实现通用限流和磁盘配额。
...
- `pytest -q tests/unit tests/api`：39 项通过。
- `npm run build`：通过。
- Docker Compose 合并配置解析：通过，Web、API、Redis 默认发布地址均为 `127.0.0.1`。
- API Key、短期会话、CSRF、媒体鉴权、回环地址不绕过认证、弱密钥拒绝启动均有自动化测试。

协作收尾小任务：

- [ ] 增加浏览器级 E2E：覆盖“输入 API Key → 建立会话 → 加载受保护图片 → 注销后返回登录门”。
  后端安全边界和组件级构建已经完成；该用例用于补齐真实浏览器 Cookie 与图片加载行为的最终验收。
